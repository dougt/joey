 ...
 */
    function SVNLink($test_name = '') {
        ...
        if (!empty($path)) {
            ...
            $link = '<a class="svn" href="http://svn.mozilla.org/projects/kubla/trunk/app/'.$name.'">Test Subject</a>';
            return $link;
        }
    }
 ...
 */
	...
	function parseMessage($message) {
		if (preg_match('/(.*) at \['.str_replace('/', '\/', TESTS).'(\S+) (line (\d+))\]/i', $message, $matches)) {
			$message = $matches[1].' @ <a href="http://svn.mozilla.org/addons/trunk/site/app/tests/'.$matches[2].'#'.$matches[4].'">'.$matches[3].'</a> ('.memory_get_usage().')<br>';
		}
		return $message;
	}
